...
class CandleAggregator:
    ...
    def update_candle(self, product_id: str, candle: Candle) -> List[Tuple[Granularity, Candle, bool]]:
        ...
        for granularity in Granularity:
            interval_timestamp = self._get_interval_timestamp(
                candle.first_timestamp, granularity)
            ...
            is_complete = (candle.last_timestamp -
                           interval_timestamp) >= granularity.seconds
            if is_complete:
...
class CoinbaseWebSocketClient:
    ...
    async def connect(self) -> tuple[bool, str]:
        ...
        logger.info(
            f"{self.client_id}: Connecting to Coinbase WebSocket (attempt {self.connection_attempts}), products: {self.product_ids}")

        try:
            async with async_timeout.timeout(30):  # Reduced timeout
                try:
                    logger.debug(
                        f"{self.client_id}: Initiating WebSocket connection")
                    ...
                    logger.debug(
                        f"{self.client_id}: WebSocket connection established")
                except Exception as conn_err:
                    logger.error(
                        f"{self.client_id}: WebSocket connection failed: {str(conn_err)}", exc_info=True)
                    return False, f"WebSocket connection error: {str(conn_err)}"

                try:
                    logger.debug(
                        f"{self.client_id}: Preparing subscription message")
                    subscribe_message = {
                        "type": "subscribe",
                    ...
                    logger.debug(
                        f"{self.client_id}: Sending subscription message")
                    await self.websocket.send(json.dumps(subscribe_message))
                    logger.debug(
                        f"{self.client_id}: Subscription message sent")

                    subscription_timeout = time.time() + 30
                    while time.time() < subscription_timeout:
                        logger.debug(
                            f"{self.client_id}: Waiting for subscription response")
                        response = await self.websocket.recv()
                        message = json.loads(response)
                        logger.debug(
                            f"{self.client_id}: Received response: {message}")
                        ...
                        if message.get('channel') == 'subscriptions':
                            ...
                            logger.warning(
                                f"{self.client_id}: Unknown message: {message.get('type')}/{message.get('channel')}, assuming were connected")
                            self.connected = True
                            self.connection_attempts = 0
                            return True, "Connected successfully"
                        elif message.get('type') == 'error':
                            ...
                            logger.error(
                                f"{self.client_id}: Subscription error: {error_msg}")
                    ...
                    logger.error(
                        f"{self.client_id}: Error during subscription: {str(e)}", exc_info=True)
                    return False, f"Subscription error: {str(e)}"

        except asyncio.TimeoutError:
            logger.error(
                f"{self.client_id}: Overall connection timeout, products: {self.product_ids}")
            return False, "Connection timed out"

        except Exception as e:
            logger.error(
                f"{self.client_id}: Unexpected error during connect: {str(e)}", exc_info=True)
    ...
    async def _process_candle(self, candle: dict, store_historical: bool):
        ...
        try:
            ...
            candle_buffer: Candle = Candle.from_coinbase_message(candle)
            ...
            for granularity, interval_data, is_complete in updates:
                logger.debug(
                    f"Saving candle for {product_id} at {granularity.name}: {candle}")
                await self.firestore.update_live_candle(
                    f"exchanges/coinbase/products/{product_id}/intervals/{granularity.name}",
    ...
    async def listen(self):
        ...
        logger.info(
            f"{self.client_id}: Starting listen loop, products: {self.product_ids}")

        while self.should_run:
            try:
                if not self.connected:
                    logger.info(
                        f"{self.client_id}: Not connected in listen loop: Attempting to connect, products: {self.product_ids}")
                    success, status = await self.connect()
                    if not success:
                        logger.warning(
                            f"{self.client_id}: Connection failed: {status}, products: {self.product_ids}")
                        await self._close_websocket()

                        delay = min(30, self.RECONNECT_DELAY *
                                    (2 ** (self.connection_attempts - 1)))
                        logger.info(
                            f"{self.client_id}: Waiting {delay}s before reconnect attempt")
                        await asyncio.sleep(delay)

                        continue
                    else:
                        logger.info(
                            f"{self.client_id}: Connected successfully! Products: {self.product_ids}")
                ...
                logger.warning(
                    f"{self.client_id}: Listen task cancelled, products: {self.product_ids}")
                await self._close_websocket()
                ...
                if not self.should_run:
                    break
                logger.warning(
                    f"{self.client_id}: Connection error ({type(e).__name__}), initiating reconnect")
                await self._close_websocket()
                await asyncio.sleep(self.RECONNECT_DELAY)
                continue
                ...
                if not self.should_run:
                    break
                logger.error(
                    f"{self.client_id}: Error in listen loop: {e}", exc_info=True)
                await self._close_websocket()
                await asyncio.sleep(self.RECONNECT_DELAY)
    ...
    async def disconnect(self):
        ...
        try:
            ...
            logger.error(
                f"{self.client_id}: Error disconnecting websocket: {e}")
    ...
    async def _handle_message(self, raw_message: str):
        ...
        try:
            ...
            logger.debug(
                f"{self.client_id}: Message: {message}, products: {self.product_ids}")

            message_type = message.get("type")
            channel = message.get("channel")

            logger.debug(
                f"{self.client_id}: Message type: {message_type}, channel: {channel}")
            ...
            # {'channel': 'candles', 'client_id': '', 'timestamp': '2024-12-01T14:14:35.576866765Z', 'sequence_num': 5, 'events': [{'type': 'update', 'candles': [{'start': '1733062200', 'high': '97299.99', 'low': '97199.63', 'open': '97285.3', 'close': '97199.63', 'volume': '8.25478631', 'product_id': 'BTC-USD'}]}]}
                ...
                for event in events:
                    ...
                    if event_type == 'update':
                        ...
                        logger.info(
                            f"{self.client_id}: Processing candle update, products: {self.product_ids}, candle: {candle}")
                        if candle:
            ...
            logger.error(
                f"{self.client_id}: Error handling message: {e}", exc_info=True)
    ...
    async def _close_websocket(self):
        ...
        if self.websocket and not self.websocket.closed:
            try:
                ...
                logger.warning(
                    f"{self.client_id}: Error closing websocket: {e}")
        self.websocket = None
        self.connected = False
